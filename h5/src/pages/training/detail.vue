<template>
<view class="page">
  <view v-if="loadError" class="loading">
    <text>{{ loadError }}</text>
    <view class="retry-btn" @tap="loadItem(pageItemId)">点击重试</view>
  </view>
  <view v-else-if="loading || !item" class="loading">加载中...</view>
  <template v-else>
    <view class="dc">
...
const item=ref(null),loadError=ref(''),loading=ref(false),playing=ref(false),recording=ref(false),paused=ref(false),uploading=ref(false),timer=ref(0),result=ref(null),uploadProgress=ref(0)
...
let recordStartTs=0, pausedTotal=0, pauseStartTs=0, mediaInited=false
...
function replayRecording(){
  ...
  ensureMediaReady()
  if(!replayContext) return uni.showToast({ title: '当前环境不支持播放', icon: 'none' })
  if(replayStatus.value==='⏸ 暂停回放'){
    replayContext.pause()
    replayStatus.value='🔊 回放我的录音'
    return
  }
  const fullUrl=resolveMediaUrl(userAudioUrl.value)
...
async function playSample(){
  ensureMediaReady()
  if(!audioContext) return uni.showToast({ title: '当前环境不支持播放', icon: 'none' })
  if(playing.value){
    // 暂停
    audioContext.pause()
    playing.value=false
  ...
  if(item.value?.sample_audio_url){
    const fullUrl=resolveMediaUrl(item.value.sample_audio_url)
  ...
  if(item.value?.sample_text){
    ...
    try{
      ...
      if(data.audio_url){
        ...
        const fullUrl=resolveMediaUrl(data.audio_url)
        audioContext.src=fullUrl
        audioContext.play()
        playing.value=true
        sampleStatus.value='⏸ 暂停范本'
...
function toggleRecord(){
  ...
  ensureMediaReady()
  if(!recorderManager) return uni.showToast({ title: '当前环境不支持录音', icon: 'none' })
  requestMicPermission()
    .then(() => startRecord())
    .catch(() => uni.showToast({ title: '请允许浏览器使用麦克风', icon: 'none' }))
}
...
async function loadItem(id) {
  ...
  if (!itemId) {
    loadError.value = '缺少素材 ID'
    return
  }
  loading.value = true
  loadError.value = ''
  try {
    item.value = await api.get('/training/items/' + itemId)
    checkFav()
  } catch (e) {
    console.error('[detail] load failed', itemId, e)
    item.value = null
    loadError.value = '素材加载失败，请检查网络与后端服务'
  } finally {
    loading.value = false
  }
}
...
// ========== 生命周期 ==========
onLoad(async (opt) => {
  pageItemId.value = getRouteParam(opt, 'id')
  if (opt?.tk) taskIndex = parseInt(opt.tk)
  if (opt?.min) minDuration = parseInt(opt.min) || 30
  await loadItem(pageItemId.value)
  ensureMediaReady()
})
onShow(() => {
  retryPending()
  if (!item.value && !loading.value && pageItemId.value && !loadError.value) {
    loadItem(pageItemId.value)
  }
})
...
.loading{padding:80rpx 40rpx;text-align:center;color:#999;font-size:28rpx}
.retry-btn{margin-top:24rpx;color:var(--brand-primary);font-size:26rpx}
