...
<div class="tr">
  <el-card shadow="never" class="mb16">
    <el-row :gutter="12">
      <el-col :span="3"><el-select v-model="f.category" placeholder="全部分类" clearable @change="load" style="width:100%"><el-option label="基础口才" value="basic"/><el-option label="演讲实战" value="speech"/><el-option label="直播话术" value="livestream"/><el-option label="即兴表达" value="improv"/></el-select></el-col>
      <el-col :span="2"><el-select v-model="f.difficulty" placeholder="难度" clearable @change="load" style="width:100%"><el-option label="⭐" :value="1"/><el-option label="⭐⭐" :value="2"/><el-option label="⭐⭐⭐" :value="3"/></el-select></el-col>
      ...
      <el-col :span="3"><el-select v-model="f.source" placeholder="来源" clearable @change="load" style="width:100%"><el-option label="人工创建" value="manual"/><el-option label="AI生成" value="ai_generated"/></el-select></el-col>
      <el-col :span="6"><el-input v-model="f.keyword" placeholder="搜索标题或标签" clearable @keyup.enter="load"/></el-col>
      <el-col :span="4" :offset="3" style="text-align:right"><el-button type="primary" @click="open()">+ 新增训练</el-button></el-col>
    </el-row>
  </el-card>
  ...
  <el-card shadow="never">
    <el-table :data="list" stripe v-loading="loading" @selection-change="sids=$event.map(v=>v.id)" empty-text="暂无训练内容，点击上方按钮新增">
      ...
      <el-table-column label="分类" width="90"><template #default="{row}">{{cat(row.category)}}</template></el-table-column>
      <el-table-column label="难度" width="70"><template #default="{row}">{{'⭐'.repeat(row.difficulty)}}</template></el-table-column>
      <el-table-column label="来源" width="70"><template #default="{row}"><el-tag :type="row.source==='ai_generated'?'warning':''" size="small">{{row.source==='ai_generated'?'AI':'人工'}}</el-tag></template></el-table-column>
      <el-table-column prop="practice_count" label="练习次数" width="90"/>
      <el-table-column label="状态" width="75"><template #default="{row}"><el-switch :model-value="row.status==='online'" @change="tog(row)" active-color="#FF6B35" size="small"/></template></el-table-column>
    ...
    <div v-if="sids.length" class="bb"><el-button size="small" type="success" @click="batch('online')">批量上架</el-button><el-button size="small" type="danger" @click="batch('offline')">批量下架</el-button></div>
  ...
  <el-dialog v-model="dlg" :title="eid?'编辑训练题':'新增训练题'" width="640px" destroy-on-close>
    <el-form :model="form" label-width="80px">
      ...
      <el-form-item label="来源"><el-radio-group v-model="form.source"><el-radio label="manual">人工创建</el-radio><el-radio label="ai_generated">AI生成</el-radio></el-radio-group></el-form-item>
...
function reset(){Object.assign(form,{title:'',category:'',difficulty:1,tags:[],sample_text:'',sort_order:0,status:'online',source:'manual'});ti.value='';eid.value=0}
function open(row){if(row){eid.value=row.id;Object.assign(form,{title:row.title,category:row.category,difficulty:row.difficulty,tags:[...row.tags||[]],sample_text:row.sample_text,sort_order:row.sort_order,status:row.status,source:row.source||'manual'})}else reset();dlg.value=true}
